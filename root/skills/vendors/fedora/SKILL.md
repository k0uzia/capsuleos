---
name: capsuleos-vendor-fedora
description: CapsuleOS vendor fedora — distributions linux-fedora. Use when working on fedora branding, assets vendors/fedora, or any fedora simulated OS entry.
---

# Vendor — fedora

## Périmètre

| Champ | Valeur |
|-------|--------|
| Vendor | `fedora` |
| Familles | linux |
| Skill famille OS | `os-linux` |

## Distributions (registre)

- [`linux-fedora`](../distributions/linux-fedora/SKILL.md) — skill `capsuleos-distro-linux-fedora`

## Chaîne agent

1. `onboarding` → `validate-all.mjs`
2. Skill **famille** : os-linux
3. Skill **distribution** : `capsuleos-distro-<id>` pour l’entrée ciblée
4. Skill **version** si applicable : `capsuleos-version-*`
5. Skill **langage** selon fichiers touchés : `capsuleos-lang-javascript`, `capsuleos-lang-css`, …

## Assets

- Pack vendor : `usr/share/capsuleos/assets/images/vendors/fedora/`
- Ne pas créer de médias hors zones autorisées ([politique-assets.md](../../docs/politique-assets.md))

## Gates

```bash
node usr/lib/capsuleos/tools/validate-all.mjs
```

## Références

- [skills-hierarchie.md](../../docs/skills-hierarchie.md)
- [repertoire-os.md](../../docs/repertoire-os.md)

