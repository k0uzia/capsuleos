/**
 * Redimensionnement des fenêtres CapsuleOS (cross-OS).
 * API : class Resizer — instanciée par windowContainer / shells.
 */
class Resizer {
    constructor(element, options = {}) {
        this.element = element;
        this.BORDER_SIZE = options.borderSize ?? 5;
        this.resizing = false;
    ...
    checkBorder(e) {
        ...
        const boundsEl = document.getElementById('desktop') || document.querySelector('main');
        if (boundsEl) {
            const dRect = boundsEl.getBoundingClientRect();
            newLeft = Math.max(dRect.left, newLeft);
            newTop = Math.max(dRect.top, newTop);
            newWidth = Math.min(newWidth, dRect.right - newLeft);
            newHeight = Math.min(newHeight, dRect.bottom - newTop);
