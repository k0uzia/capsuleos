(() => {
    ...
    let volume = parseInt((localStorage.getItem('mint-volume') != null ? localStorage.getItem('mint-volume') : '100'), 10);
    ...
    const observer = new MutationObserver(mutations => {
        ...
        for (const m of mutations) {
            for (const node of m.addedNodes) {
                ...
                if ((node.matches == null ? void 0 : node.matches('audio, video'))) node.volume = v;(node.querySelectorAll == null ? void 0 : node.querySelectorAll('audio, video')).forEach(el => { el.volume = v; });
            }
        }
    });
