#!/usr/bin/env node
/**
 * Captures Capsule GNOME Agenda — scénarios Cal1–Cal4.
 *
 * Usage :
 *   CAPSULE_HTTP_BASE=http://127.0.0.1:5501 node usr/lib/capsuleos/tools/lab/capture-capsule-calendar-views.mjs --id linux-alma
 */
import fs from 'fs';
...
const openCalendar = async (page) => {
  ...
  await page.waitForFunction(
    () => {
      const win = document.querySelector('.windowElement[data-link="calendar"]');
...
const SCENARIOS = [
  {
    file: 'rocky-capsule-dark-calendar.png',
    run: async (page) => {
      await openCalendar(page);
    },
  },
  {
    file: 'rocky-capsule-dark-calendar-month.png',
    run: async (page) => {
      await openCalendar(page);
  ...
  {
    file: 'rocky-capsule-dark-calendar-event-added.png',
    run: async (page) => {
      await openCalendar(page);
  ...
  {
    file: 'rocky-capsule-dark-calendar-week.png',
    run: async (page) => {
      await openCalendar(page);
  ...
  {
    file: 'rocky-capsule-dark-calendar-next-month.png',
    run: async (page) => {
      await openCalendar(page);
...
const main = async () => {
  ...
  for (const scenario of SCENARIOS) {
    ...
    const win = page.locator('.windowElement[data-link="calendar"]');
  ...
  await browser.close();
  console.log(`✓ capture-capsule-calendar-views ${opts.id} — ${SCENARIOS.length} PNG → ${outDir}`);
};
