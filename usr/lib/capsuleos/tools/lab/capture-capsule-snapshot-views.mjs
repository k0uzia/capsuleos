#!/usr/bin/env node
/** Captures Capsule GNOME Snapshot — scénarios Sn1–Sn4. */
import fs from 'fs';
...
const openSnapshot = async (page) => {
  await page.evaluate(() => window.openWindowByDataLink?.('snapshot'));
...
const SCENARIOS = [
  { file: 'rocky-capsule-dark-snapshot.png', run: openSnapshot },
  { file: 'rocky-capsule-dark-snapshot-photo.png', run: openSnapshot },
  { file: 'rocky-capsule-dark-snapshot-video.png', run: async (page) => { await openSnapshot(page); await page.click('[data-snapshot-gnome-mode="video"]'); await sleep(page, 150); } },
  { file: 'rocky-capsule-dark-snapshot-empty.png', run: openSnapshot },
];
...
const main = async () => {
  ...
  for (const scenario of SCENARIOS) {
    ...
    await page.locator('.windowElement[data-link="snapshot"]').screenshot({ path: path.join(paths.capsuleCapturesDir, scenario.file) });
    process.stdout.write(`  ✓ ${scenario.file}\n`);
    await page.close();
  }
  await browser.close();
  console.log(`✓ capture-capsule-snapshot-views ${opts.id} — ${SCENARIOS.length} PNG`);
};
