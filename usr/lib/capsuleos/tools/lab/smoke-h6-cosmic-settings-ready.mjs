#!/usr/bin/env node
/**
 * Gate pré-H6 COSMIC — contrat τ (h6Ready) + chaîne Paramètres + parité Vp.
 *
 * Usage :
 *   node usr/lib/capsuleos/tools/lab/smoke-h6-cosmic-settings-ready.mjs --id linux-popos
 */
import fs from 'fs';
...
if (!tail.h6Ready) {
  console.error(`✗ h6Ready=false — nextH5: ${(tail.nextH5 || []).map((n) => n.target).join(', ') || '—'}`);
  process.exit(1);
}
...
run('verify-cosmic-settings-parity-chain.mjs');
run('verify-gnome-settings-parity-chain.mjs', ['--strict']);
process.stdout.write(`○ smoke-h5-p0-shell ${registry} ignoré (toolkit COSMIC)\n`);
...
fs.writeFileSync(statePath, `${JSON.stringify({
  ...
  gate: 'smoke-h6-cosmic-settings-ready.mjs',
  generatedAt: new Date().toISOString(),
}, null, 2)}\n`);

process.stdout.write(`✓ smoke-h6-cosmic-settings-ready ${registry} OK — ${statePath.replace(`${ROOT}/`, '')}\n`);
