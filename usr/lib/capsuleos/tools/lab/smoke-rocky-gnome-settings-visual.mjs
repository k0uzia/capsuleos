#!/usr/bin/env node
/**
 * Smoke enquête visuelle Paramètres — Rocky (V / Vc / Vp P0).
 */
import fs from 'fs';
...
if (!fs.existsSync(path.join(ROOT, 'root/tools/lab/gnome-settings-visual-investigation-matrix-rocky.json'))) {
  errors.push('gnome-settings-visual-investigation-matrix-rocky.json absent');
}

if (errors.length) {
  console.error('smoke-rocky-gnome-settings-visual — échec\n');
...
const inv = JSON.parse(fs.readFileSync(invPath, 'utf8'));
console.log(
  `✓ smoke-rocky-gnome-settings-visual OK — P0 doc=${countP0Documented(inv)} `
